...
# Session additions — 2026-06-12 (rebrand + Pebble Time Round)

Both implemented and merged to the fork's `main`:

## ✅ Claude rebrand — commit `4564fcd`
- `ACCENT_COLOUR` (style.h) pink → **GColorSunsetOrange** (#FF5555, nearest
  Pebble palette colour to Claude's #D97757); every branded surface follows.
- All pony artwork → a 12-ray **Claude starburst**, generated by
  `tools/claude-brand/generate.py` (committed, re-runnable): launcher menu
  icon, root-screen mascot (+emery), sleeping/failed result images, loading
  spinner + ringing pulse animations, about-screen bitmaps. Resource names,
  viewboxes and memory formats unchanged → zero C changes for the art swap;
  generated PDC binaries validated against the parser used to decode the
  originals.
- Error result panes moved SunsetOrange → red (the old error colour is now
  the brand colour).
- ⚠️ Note: the Claude spark/colours are Anthropic trademarks — fine for a
  personal sideloaded build, but don't publish this branding to the Rebble
  appstore.

## ✅ Pebble Time Round (chalk) support — commit `8181ed7`
- `chalk` added to `targetPlatforms` + colour-scoped resources (menu icons,
  about image). PTR has a mic, so dictation works.
- Round layout fixes (all `PBL_IF_ROUND_ELSE`/`PBL_ROUND`-guarded; rect
  platforms bit-identical): clock/version/mascot repositioned on the root
  screen, centre-focused menus, chat column inset 14px/side, wider margins
  in about/legal/quota/feedback/report/consent, ring-screen pulse inset.
- **Validated geometrically only** — run `pebble build && pebble install
  --emulator chalk` (cloud.repebble.com) and eyeball each screen before
  sideloading to the PTR. Expect to tune a margin or two.
